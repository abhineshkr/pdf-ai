...
from app.pdf.postprocessors.editor import load_editing_model
...
def setup_detection_model(device=None, dtype=None):
    if device:
        model = model.load_model(device=device, dtype=dtype)
    else:
...
def setup_layout_model(device=None, dtype=None):
    if device:
        model = model.load_model(checkpoint=settings.LAYOUT_MODEL_CHECKPOINT, device=device, dtype=dtype)
    else:
        model = model.load_model(checkpoint=settings.LAYOUT_MODEL_CHECKPOINT)
    processor = model.load_processor(checkpoint=settings.LAYOUT_MODEL_CHECKPOINT)
    model.processor = processor
    return model
